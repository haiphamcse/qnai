...
def balance_loss(y: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:

    loss = -y * tf.math.log(y_pred) - (1. - y)*tf.math.log(1-y_pred) * y_pred
...
def train(model, epochs: int, batch_size: int, lr: float, beta_adam = [0.9, 0.99]):
    ...
    history = model.fit(x = train_features[1], y = train_labels, validation_data = (test_features[1], test_labels), epochs = epochs, batch_size = batch_size)
